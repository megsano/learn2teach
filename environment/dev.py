...
if __name__ == "__main__":
    deep = Engine(depth=11)
    pos = game.Position(util.initial, 0, (True,True), (True,True), 0, 0)
    moves_list = ['e2e4', 'c7c5', 'd1g4', 'g8f6', 'b2c3', 'f6g4']
    deep.setposition(moves_list)
    print(deep.bestmove()['info'])
    print('****************')
    print(deep.bestmove()['ponder'])
    for i, move in enumerate(moves_list):
        isStudent = i % 2 == 1
        dqn_move = (util.convert_to_nums(move[0:2]),util.convert_to_nums(move[2:]))
        pos = pos.move(dqn_move, isStudent)
        pos = pos.rotate()
    game.print_pos(pos)
    # with_teacher = True
