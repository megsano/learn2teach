...
class Position(namedtuple('Position', 'board score wc bc ep kp')):
    ...
    def getNewState(self, move):
        ...
        # new_score = str(score)
        # Actual move
        new_state = put(new_state, j, new_state[i])
        new_state = put(new_state, i, '.')
        # Castling rights, we move the rook or capture the opponent's
        if i == A1: new_wc = (False, new_wc[1])
        if i == H1: new_wc = (wc[0], False)
        if j == A8: new_bc = (bc[0], False)
        if j == H8: new_bc = (False, bc[1])
        # Castling
        if p == 'K':
            new_wc = (False, False)
            if abs(j-i) == 2:
                new_kp = (i+j)//2
                new_state = put(new_state, A1 if j < i else H1, '.')
                new_state = put(new_state, new_kp, 'R')
        # Pawn promotion, double move and en passant capture
        if p == 'P':
            if A8 <= j <= H8:
                new_state = put(new_state, j, 'Q')
            if j - i == 2*N:
                new_ep = i + N
            if j - i in (N+W, N+E) and q == '.':
